...
Three runnable native examples, ordered by complexity:
...
A browser example template (worker entry, OPFS install, schema bootstrap, message-port API) ships separately — see the framework repo's `examples/` directory for the latest reference implementation.
