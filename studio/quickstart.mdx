...
<Warning>
**Phase 1b (current)** completes Studio's read story: SQLite via
`rusqlite`, relation traversal (`/records/:pk/rel/:field`), API-backed
list/get against deployed cratestack services, and a Leptos+Trunk web
UI you can run locally. Mutations land in Phase 3.

The endpoints are documented in detail in
[Studio Read API](./read-api), and the browser UI in
[Studio UI](./ui).
</Warning>
...
| 2     | `studio eject` + bundled UI via `rust-embed` (single-binary distribution)                            | Pending       |
