...
<Warning>
**Phase 2 (current)** makes Studio distributable:
[`studio eject`](./eject#eject-the-ui-sources) writes the UI sources to
a writable directory you can fork, and the
[`embed-ui` cargo feature](./eject#bundle-the-ui-into-the-binary)
bundles the SPA into the Studio binary so you can hand `cratestack`
to a colleague without Trunk in the picture. The read API
(Phase 1a/1b) is otherwise unchanged. Mutations land in Phase 3.
...
| 1c    | Typed relation-picker in the UI, polish on the drawer, JSON field editors                            | Pending       |
