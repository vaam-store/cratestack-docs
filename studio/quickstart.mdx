...
<Warning>
**Phase 3 (current)** adds writes: `POST` / `PATCH` / `DELETE`
record endpoints, server-side
[validator pass-through](./write-api#validators) with structured
per-field errors, and a `mode = "ro"` gate that returns
**403 FORBIDDEN**. The UI gets a typed relation picker, an inline
create form, an edit-in-place drawer, and a delete confirmation.

Reference pages: [Studio Read API](./read-api),
[Studio Write API](./write-api),
[Studio UI](./ui), [Studio Eject](./eject).
</Warning>
...
| 1d    | Typed pickers for enums, rich editors for JSON / DateTime / Decimal in the create + edit forms       | Pending       |
| 4     | SQL preview, drift, CSV/JSON export, schema search, audit log, SQLSTATE → VALIDATION_ERROR mapping   | Pending       |
| 3     | Write path: create / edit / delete with policy validation                                            | Pending       |
| 4     | SQL preview, drift, CSV/JSON export, schema search, audit log                                        | Pending       |
